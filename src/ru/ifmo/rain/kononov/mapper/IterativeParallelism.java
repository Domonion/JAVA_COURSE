...
public class IterativeParallelism implements ListIP {
    ...
    /**
     * Constructor for IterativeParallelism with mapper.
     *
    ...
    private <T, R> List<R> resolve(int allowedThreads, final List<? extends T> values, final Function<Stream<? extends T>, ? extends R> task) throws InterruptedException {
        ...
        final List<Stream<? extends T>> subTasks = new ArrayList<>(Collections.nCopies(usingThreads, null));
        int l, r = 0;
        for (int i = 0; i < usingThreads; ++i) {
            l = r;
            r = l + block + (i < tail ? 1 : 0);
            subTasks.set(i, values.subList(l, r).stream());
        }
        if (mapper != null) {
    ...
    private <T, R> List<R> map(final int usingThreads, final Function<Stream<? extends T>, ? extends R> task, final List<Stream<? extends T>> subTasks) {
        final AbstractList<R> result = new LinkedList<>(Collections.nCopies(usingThreads, null));
        final List<Thread> threads = new ArrayList<>();
        for (int i = 0; i < usingThreads; ++i) {
            ThreadUtils.addWithStart(threads, new Thread(ThreadUtils.getRunnable(result, i, task, subTasks)));
        }
